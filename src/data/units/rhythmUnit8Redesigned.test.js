...
describe('Rhythm Unit 8 — Syncopation', () => {
  ...
  it('first node is syncopation discovery', () => {
    ...
    expect(firstNode.newContentDescription).toBe('Syncopation: Tap between the beats!');
  ...
  it('third node introduces dotted quarter-eighth', () => {
    ...
    expect(thirdNode.newContentDescription).toBe('Dotted Quarter-Eighth Syncopation');
  });
...
describe('Rhythm Unit 8 — Boss Challenge (RADV-04)', () => {
  ...
  it('boss mixes both 6/8 and 4/4 content', () => {
    const timeSignatures = bossNode.exercises.map((ex) => ex.config.timeSignature);
