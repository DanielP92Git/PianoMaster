...
describe("Rhythm Unit 8 — Syncopation", () => {
  ...
  it("first node focusDurations is empty (C-04 fix)", () => {
    expect(rhythmUnit8Nodes[0].rhythmConfig.focusDurations).toEqual([]);
  });

  it("third node introduces dotted quarter-eighth syncopation", () => {
    ...
    expect(thirdNode.newContentDescription).toBe(
      "Dotted Quarter-Eighth Syncopation Pattern"
    );
  });

  it("third node focusDurations is empty (C-05 fix)", () => {
    expect(rhythmUnit8Nodes[2].rhythmConfig.focusDurations).toEqual([]);
  });
  ...
  it("regular node exercise types match remediated policy (G-40 through G-44)", () => {
    const regularNodes = rhythmUnit8Nodes.slice(0, 6);
    const types = regularNodes.map((n) => n.exercises[0].type);
    expect(types).toEqual([
      EXERCISE_TYPES.RHYTHM_TAP, // rhythm_8_1 (G-40: RHYTHM -> RHYTHM_TAP)
      EXERCISE_TYPES.RHYTHM_TAP, // rhythm_8_2 (G-41: RHYTHM -> RHYTHM_TAP)
      EXERCISE_TYPES.RHYTHM_TAP, // rhythm_8_3 (already correct)
      EXERCISE_TYPES.RHYTHM_TAP, // rhythm_8_4 (G-42: RHYTHM_DICTATION -> RHYTHM_TAP)
      EXERCISE_TYPES.RHYTHM_DICTATION, // rhythm_8_5 (G-43: RHYTHM_TAP -> RHYTHM_DICTATION)
      EXERCISE_TYPES.ARCADE_RHYTHM, // rhythm_8_6 (G-44: RHYTHM -> ARCADE_RHYTHM)
    ]);
  });
