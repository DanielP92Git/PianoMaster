...
import { EXERCISE_TYPES, NODE_CATEGORIES } from "./constants.js";
import { TRAIL_TAB_CONFIGS } from "./trailTabConfigs.js";
...
describe("First trail nodes use note_catch exercise type (REQ-05)", () => {
  ...
  it("treble_1_1 config has targetNote and distractorNotes", () => {
    ...
    expect(config.distractorNotes).toEqual(
      expect.arrayContaining(["D4", "E4"])
    );
...
describe("TRAIL_TAB_CONFIGS (INFRA-04)", () => {
  ...
  it("tab order is treble, bass, rhythm, ear_training (per D-11)", () => {
    expect(TRAIL_TAB_CONFIGS.map((t) => t.id)).toEqual([
      "treble",
      "bass",
      "rhythm",
      "ear_training",
