...
function StatusBadge({ status, t }) {
  const statusConfig = {
    active: {
      className: "bg-green-400/20 border-green-400/30 text-green-300",
      labelKey: "parentPortal.statusActive",
    },
    on_trial: {
      className: "bg-blue-400/20 border-blue-400/30 text-blue-300",
      labelKey: "parentPortal.statusOnTrial",
    },
    cancelled: {
      className: "bg-amber-400/20 border-amber-400/30 text-amber-300",
      labelKey: "parentPortal.statusCancelled",
    },
    past_due: {
      className: "bg-red-400/20 border-red-400/30 text-red-300",
      labelKey: "parentPortal.statusPastDue",
    },
    expired: {
      className: "bg-gray-400/20 border-gray-400/30 text-gray-300",
      labelKey: "parentPortal.statusExpired",
    },
  };
...
export default function ParentPortalPage() {
  ...
  const { preferences, updatePreference, updateNotificationType } =
    useSettings();
  ...
  const formattedAmount = formatAmount(
    effectiveDetail?.amountCents,
    effectiveDetail?.currency,
    locale
  );

  const billingLabel =
    effectiveDetail?.billingPeriod === "yearly"
