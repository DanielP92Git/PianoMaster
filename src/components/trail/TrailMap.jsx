...
const TrailMap = () => {
  ...
  const nodesWithBossByTab = useMemo(() => {
    return Object.fromEntries(
      TRAIL_TAB_CONFIGS.map((tab) => {
        const categoryNodes = getNodesByCategory(
          NODE_CATEGORIES[tab.categoryKey]
        );
        const tabBosses = bossNodes.filter((b) =>
          b.id.startsWith(tab.bossPrefix)
        );
        const merged = [...categoryNodes, ...tabBosses].sort(
          (a, b) => a.order - b.order
        );
  ...
  const activeTabConfig =
    TRAIL_TAB_CONFIGS.find((t) => t.id === activeTab) || TRAIL_TAB_CONFIGS[0];
  ...
  const { nodesByUnit, units } = useMemo(() => {
    ...
    Object.values(grouped).forEach((arr) =>
      arr.sort((a, b) => (a.orderInUnit || 0) - (b.orderInUnit || 0))
    );
    ...
    const unitsList = unitKeys.map((unitNum) => {
      ...
      return unitKey
        ? UNITS[unitKey]
        : { order: unitNum, name: `Unit ${unitNum}`, icon: "📚" };
    });
  ...
  const handleTabKeyDown = (e, index) => {
    ...
    if (e.key === prevKey) {
      e.preventDefault();
      const prevIndex =
        (index - 1 + TRAIL_TAB_CONFIGS.length) % TRAIL_TAB_CONFIGS.length;
  ...
  return (
    <div className="relative z-10 mx-auto max-w-6xl space-y-4 p-4 sm:space-y-6 sm:p-6">
      ...
      <div className="mx-auto max-w-md rounded-2xl bg-black/30 p-3 backdrop-blur-sm">
        <div
          role="tablist"
          aria-label={t("tabs.ariaLabel", { defaultValue: "Learning paths" })}
          ...
          {TRAIL_TAB_CONFIGS.map((tab, index) => {
            ...
            return (
              <button
                ...
                className={`flex h-[80px] w-[72px] flex-col items-center justify-center rounded-2xl text-xs font-bold transition-all duration-200 sm:h-[88px] sm:w-[80px] ${
                  isActive
                    ? `${tab.colorActive} border-2 ${tab.colorBorder} ${tab.colorGlow} text-white`
                    : `${tab.colorInactive} border border-white/10 text-white/80 hover:border-white/20`
                } `}
              >
                <TabIcon className="mb-1 h-7 w-7 drop-shadow sm:h-8 sm:w-8" />
      ...
      <div
        ...
        aria-label={t(`tabs.${activeTab}Panel`, {
          defaultValue: `${activeTab} learning path`,
        })}
      >
