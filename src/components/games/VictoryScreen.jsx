...
const VictoryScreen = ({
  ...
  const levelTitle = levelProgressData?.currentLevel?.isPrestige
    ? t("xpLevels.prestigeTitle", {
        tier: levelProgressData.currentLevel.prestigeTier,
      })
  ...
  return (
    <div className="victory-bg fixed inset-0 z-[9999] flex items-center justify-center overflow-y-auto p-2 sm:p-4">
      ...
      <div className="my-auto flex w-full max-w-md flex-col items-center gap-2 landscape:max-w-lg">
        {/* Rate limit banner */}
        ...
        <div className="flex w-full justify-center">
          <svg
            viewBox="0 0 300 90"
            className="h-auto w-full max-w-xs overflow-visible sm:max-w-sm"
            aria-label={
              rateLimited
        ...
        {nodeId && xpData && xpData.totalXP > 0 && (
          <div className="relative inline-flex items-center gap-2 rounded-full border-2 border-blue-400/50 bg-gradient-to-b from-indigo-900/90 to-slate-900/90 px-5 py-1.5 shadow-[0_0_16px_rgba(96,165,250,0.35),inset_0_1px_0_rgba(255,255,255,0.1)]">
            <p
              className="bg-gradient-to-r from-blue-200 to-cyan-200 bg-clip-text text-lg font-bold text-transparent"
              style={{ fontFamily: "'Fredoka One', 'Fredoka', cursive" }}
            >
              {t("victory.xpEarned", { xp: animatedXPGain })}
            </p>
            {comebackActive && (
              <span className="inline-flex items-center rounded-full border border-amber-400/40 bg-amber-500/30 px-2 py-0.5 text-xs font-bold text-amber-300">
                2x
              </span>
            )}
          </div>
        ...
        {challengeMode && challengeXpReward && (
          <div className="relative inline-flex items-center gap-2 rounded-full border-2 border-amber-400/50 bg-gradient-to-b from-amber-600/90 to-orange-900/90 px-5 py-1.5 shadow-[0_0_16px_rgba(251,191,36,0.35),inset_0_1px_0_rgba(255,255,255,0.1)]">
            <Zap className="h-4 w-4 text-amber-300" />
            <p
              className="bg-gradient-to-r from-amber-200 to-yellow-200 bg-clip-text text-lg font-bold text-transparent"
              style={{ fontFamily: "'Fredoka One', 'Fredoka', cursive" }}
            >
              {t("dashboard.dailyChallenge.bonusXp", {
                xp: challengeXpReward,
                defaultValue: `Challenge Bonus: +${challengeXpReward} XP`,
              })}
            </p>
          </div>
        )}
        ...
        {xpData?.leveledUp && (
          <span
            className={`inline-flex items-center gap-1 rounded-full px-3 py-0.5 text-sm font-semibold ${
              isPrestige
                ? "border border-amber-400/40 bg-amber-500/30 text-amber-200"
                : "border border-purple-400/40 bg-purple-500/30 text-purple-200"
            }`}
          >
            {xpData.newLevel === 30
