...
/**
 * Determines which C-root octave to display for the 1-octave SVG keyboard
 * so that both note1 and note2 are visible within the 12-key span.
 *
 * Strategy:
 * - Find the lower of the two notes
 * - Find the C that starts the octave containing that lower note
 * - Verify the higher note is within 12 semitones; if not, shift down one white key
 *
 * @param {string} note1 - First note name (e.g., 'C4')
 * @param {string} note2 - Second note name (e.g., 'E4')
 * @returns {string} The C-root of the octave to display (e.g., 'C4')
 */
export function getDisplayOctaveRoot(note1, note2) {
  const idx1 = NOTE_ORDER.indexOf(note1);
  const idx2 = NOTE_ORDER.indexOf(note2);

  if (idx1 === -1 || idx2 === -1) return 'C4';

  const lowIdx = Math.min(idx1, idx2);

  // Find the C that starts this octave
  // NOTE_ORDER is: C3(0), C#3(1), D3(2)... B3(11), C4(12)... B4(23)
  // So C3 is at index 0, C4 is at index 12 — each octave is 12 semitones
  const octaveStartIdx = lowIdx - (lowIdx % 12);
  const octaveRoot = NOTE_ORDER[octaveStartIdx];

  // Verify both notes fit within the 12-key span from this root
  const highIdx = Math.max(idx1, idx2);
  if (highIdx <= octaveStartIdx + 12) {
    return octaveRoot;
  }

  // Notes span across octave boundary — use the lower note's octave root
  // clamped to available notes
  const clampedIdx = Math.max(0, Math.min(NOTE_ORDER.length - 13, octaveStartIdx));
  return NOTE_ORDER[clampedIdx];
}
