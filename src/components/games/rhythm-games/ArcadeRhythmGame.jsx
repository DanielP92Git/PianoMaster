...
function getTileColorClass(durationUnits, isRest) {
  ...
  if (durationUnits >= 16) return 'bg-violet-400/80 border border-violet-300/60';
  if (durationUnits >= 8) return 'bg-emerald-400/80 border border-emerald-300/60';
...
function ArcadeRhythmGame() {
  ...
  // RAF animation loop
  // ---------------------------------------------------------------------------
  const startRafLoop = useCallback((tileDefinitions) => {
   ...
   */
  const finishPattern = useCallback((score) => {
    cancelAllTimers();
    setGamePhase(GAME_PHASES.FEEDBACK);
  ...
  // Countdown phase
  // ---------------------------------------------------------------------------
  const startCountdown = useCallback((beats) => {
  ...
  // Playing phase
  // ---------------------------------------------------------------------------
  const startPlayingPhase = useCallback((beats) => {
    ...
    const tileDefinitions = buildTilesFromBeats(beats, playbackStartTime, tempo);
    ...
    const beatDuration = 60 / tempo;
    const sixteenthDuration = beatDuration / 4;
    const totalDuration = beats.reduce((sum, b) => sum + b.durationUnits * sixteenthDuration, 0);
    ...
    feedbackTimeoutRef.current = setTimeout(() => {
      if (gamePhaseRef.current === GAME_PHASES.PLAYING) {
        ...
        finishPattern(Math.max(0, Math.round((hitCount / Math.max(nonRestCount, 1)) * 100)));
      }
    }, patternEndTime);
  ...
  const handleHitZoneTap = useCallback(() => {
    ...
    if (tileIdx !== -1) {
      ...
      if (tileEl) {
        if (quality === 'GOOD') {
          tileEl.style.transition = reducedMotion ? 'none' : 'opacity 200ms ease-out';
          tileEl.style.opacity = '0';
        ...
        if (node && node.exercises && node.exercises[nextIndex]) {
          ...
          switch (nextExercise.type) {
            ...
            case 'rhythm_dictation':
              ...
              break;
            case 'arcade_rhythm':
              navigate('/rhythm-mode/arcade-rhythm-game', { state: navState, replace: true });
              window.location.reload();
              break;
            case 'boss_challenge':
            ...
            case 'pitch_comparison':
              navigate('/notes-master-mode/pitch-comparison-game', { state: navState });
              break;
            case 'interval_id':
              navigate('/notes-master-mode/interval-game', { state: navState });
              break;
            default:
              navigate('/trail');
          }
  ...
  return (
    ...
    >
      ...
      <main className="relative flex-1 overflow-hidden">
          ...
          <div className="flex h-full flex-col items-center justify-center gap-6 px-4">
            <div className="text-center">
              ...
              <p className="mt-2 text-white/70 text-sm">
                {t('games.arcadeRhythm.subtitle', 'Tap the hit zone when tiles arrive!')}
              </p>
            </div>
            <button
              onClick={startGame}
        ...
        {/* ---------------------------------------------------------------- */}
        {(gamePhase === GAME_PHASES.PLAYING || gamePhase === GAME_PHASES.COUNTDOWN || gamePhase === GAME_PHASES.FEEDBACK) && (
          <div
