...
vi.mock("react-i18next", () => ({
  useTranslation: vi.fn(() => ({
    t: (key, fallback, opts) => {
      ...
      if (key === "game.pulse.ariaLabel") return "Pulse exercise — tap with the beat";
      return key;
    },
    i18n: { language: "en" },
  })),
...
vi.mock("../../RhythmPatternGenerator", () => ({
  TIME_SIGNATURES: {
    ...
    SIX_EIGHT: { beats: 2, name: "6/8", measureLength: 12, strongBeats: [0, 3], subdivisions: 6 },
  },
}));
...
vi.mock("../../components", () => ({
  MetronomeDisplay: ({ isActive, currentBeat }) => (
    <div data-testid="metronome-display" data-active={String(isActive)} data-beat={currentBeat} />
  ),
}));
...
describe("PulseQuestion", () => {
  ...
  it("renders instruction text when disabled (waiting state)", () => {
    ...
    expect(
      screen.getByText("Tap with the beat!")
    ).toBeInTheDocument();
  });
  ...
  it("accepts disabled prop and does not auto-start when disabled=true", () => {
    ...
    const { container } = render(<PulseQuestion {...defaultProps} disabled={true} />);
