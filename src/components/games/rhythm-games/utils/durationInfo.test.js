...
import { DURATION_INFO, ALL_DURATION_CODES, generateQuestions } from './durationInfo.js';
...
describe('generateQuestions', () => {
  ...
  it('every question choices array contains the correct answer exactly once', () => {
    const questions = generateQuestions(['q', 'h', 'w'], ALL_DURATION_CODES, 10);
    questions.forEach((q) => {
  ...
  it('with syllable dedup, filters distractors with duplicate syllables', () => {
    ...
    const questions = generateQuestions(['h'], ALL_DURATION_CODES, 5, { dedupSyllables: true });
    questions.forEach((q) => {
