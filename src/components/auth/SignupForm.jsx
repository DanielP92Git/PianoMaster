...
function SignupForm({ onBackToLogin }) {
  // Stage state: 'age' | 'parent-email' | 'details'
  const [step, setStep] = useState("age");
  const [dobData, setDobData] = useState(null); // { dob: Date, isUnder13: boolean }
  ...
  const handleAgeSubmit = (data) => {
    setDobData(data);
    if (data.isUnder13) {
      // Under-13 needs parent email
      setStep("parent-email");
    } else {
      // 13+ goes directly to account details
      setStep("details");
    }
  };
  ...
  const getSubtitle = () => {
    switch (step) {
      ...
      case "parent-email":
        return "One more step for your safety";
      case "details":
      default:
