    ...
    <div className="rounded-3xl border border-white/10 bg-white/5 p-6 shadow-[0_0_0_1px_rgba(255,255,255,0.06),0_0_24px_rgba(99,102,241,0.22)] backdrop-blur-md">
      ...
      >
        ...
        <div className="flex items-center gap-2">
          ...
          {allCompleted && <Trophy className="h-5 w-5 text-amber-400" />}
        </div>
      </div>
          ...
          return (
            <div
              ...
              className={`relative overflow-hidden rounded-xl border border-white/10 bg-white/5 p-4 transition-all duration-300 ${
                goal.completed ? "border-green-400/30 bg-green-500/10" : ""
              }`}
            >
              <div
              ...
              >
                ...
                <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full border-2 border-white/20 bg-white/5 shadow-[0_0_12px_rgba(99,102,241,0.2)]">
                  {goal.completed ? (
                    ...
                    />
                  ) : iconConfig.type === "goldstar" ? (
                    <GoldStar size={24} filled glow />
                  ) : iconConfig.type === "svg" ? (
                ...
                <div className={`flex-1 ${isRTL ? "text-right" : ""}`}>
                  <div className="flex items-start justify-between gap-2">
                    <div className="flex-1">
                      <div
                        className={`text-sm font-bold ${goal.completed ? "text-green-300" : "text-white/90"}`}
                      >
                        {goal.nameKey
                      ...
                      <div className="mt-0.5 text-xs text-white/60">
                        {goal.descriptionKey
                          ? t(
                              `dashboard.dailyGoals.goals.${goal.descriptionKey}.description`,
                              { target: goal.target }
                            )
      ...
      {allCompleted && (
        <div className="mt-4 rounded-xl border border-green-400/30 bg-green-500/10 p-3 text-center">
          <div className="text-sm font-semibold text-green-300">
            {t("dashboard.dailyGoals.allComplete", {
              defaultValue: "All goals completed today!",
            })}
          </div>
        </div>
      )}
      ...
      {goals.length === 0 && (
        <div className="py-8 text-center">
          ...
          <p className="text-sm text-white/60">
            {t("dashboard.dailyGoals.noGoals", {
              defaultValue: "No goals available",
            })}
          </p>
        </div>
      )}
