-- Fix notifications policy to allow teachers to message connected students
-- This allows teachers to send notifications to students they have connections with
...
CREATE POLICY "Teachers can create notifications for their students" ON notifications
    FOR INSERT WITH CHECK (
        ...
        (
            -- Teachers can message students they are connected with
            recipient_id IN (
...
'Allows teachers to send notifications to connected students via teacher_student_connections or enrolled students via class_enrollments'; 