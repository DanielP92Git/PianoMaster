...
1. Query current RLS policies using Supabase MCP tool:
   Run this query via `mcp__supabase__execute_sql` MCP tool:
```sql
...
1. Query current SECURITY DEFINER functions using Supabase MCP tool:
   Run this query via `mcp__supabase__execute_sql` MCP tool:
```sql
