...
This centralizes authorization logic and makes it easy to add features like rate limiting or violation logging later.
  ...
  <verify>
Run: `cat src/services/authorizationUtils.js | grep -c "verifyStudentDataAccess\|getCurrentUserId"` should return 2+ (function definitions).
Run: `cat src/services/authorizationUtils.js | grep -c "throw new Error"` should return 3+ (error cases).
  </verify>
  <done>
File exists with verifyStudentDataAccess and getCurrentUserId functions. Both throw descriptive errors on failure. Teacher relationship check uses teacher_student_connections table with status='accepted'.
  </done>
</task>
