...
2. Modal structure:
   ...
   - "Download JSON" button that:
     a. Calls downloadStudentDataJSON(studentId)
...
Props: { isOpen, onClose, student: { student_id, student_name } }
  </action>
  <verify>File exists with proper imports from dataExportService.js, renders Modal with download button</verify>
  <done>DataExportModal.jsx exists, imports downloadStudentDataJSON and getDataSummary, renders download UI</done>
</task>
...
2. Modal has two modes based on getAccountDeletionStatus():
   ...
   **Mode A - Not pending deletion:**
   ...
   - Name confirmation input: "Type {student_name} to confirm"
   - Disabled "Delete Account" button until name matches (case-insensitive)
   - On confirm: call requestAccountDeletion(studentId, confirmationName)
   ...
   **Mode B - Pending deletion (isPendingDeletion=true):**
   ...
   - On cancel: call cancelDeletionRequest(studentId)
...
Props: { isOpen, onClose, student: { student_id, student_name }, onDeletionRequested }
  </action>
  <verify>File exists with proper imports from accountDeletionService.js, has name confirmation and grace period UI</verify>
  <done>AccountDeletionModal.jsx exists, imports deletion service functions, renders confirmation flow with name input</done>
</task>
...
1. Add imports at top of file:
   ...
   ```
   Also add Download and AlertTriangle icons from lucide-react.

2. Add state variables in TeacherDashboard component (around line 1430 with other modal states):
   ```javascript
...
3. Add handler functions (near other handlers around line 1890):
   ```javascript
...
4. Add two new action buttons to student cards (around line 2467, after Edit3 button, before the delete button):
   ```jsx
   <button
     onClick={(e) => {
       e.stopPropagation();
...
5. Render modals at end of component (after other modals, before closing fragment):
   ```jsx
   <DataExportModal
...
6. IMPORTANT: Keep the existing Trash2 button (remove from teacher connection) - this is different from account deletion.
