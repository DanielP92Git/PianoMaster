# Requirements: PianoApp

**Defined:** 2026-04-05
**Core Value:** Children's data must be protected and inaccessible to unauthorized users

## v3.1 Requirements

Requirements for milestone v3.1 Trail-First Navigation. Each maps to roadmap phases.

### Navigation

- [ ] **NAV-01**: Trail is accessible from the desktop sidebar as a primary navigation item
- [ ] **NAV-02**: Trail is accessible from the mobile bottom tab bar as a primary navigation item
- [ ] **NAV-03**: Returning students land on the trail map by default after login (not the dashboard)
- [ ] **NAV-04**: Navigation highlight/active state correctly reflects the current page (trail, dashboard, etc.)

### Dashboard

- [ ] **DASH-01**: Dashboard is redesigned as a compact stats/overview hub (not the primary destination)
- [ ] **DASH-02**: PlayNextButton ("Continue/Begin Journey") hero CTA is removed from the dashboard
- [ ] **DASH-03**: Existing dashboard cards (streak, XP, daily goals, practice log, daily message) are retained in the compact layout
- [ ] **DASH-04**: Dashboard is accessible from sidebar and mobile bottom tabs as a secondary nav item

### Post-Game

- [x] **POST-01**: VictoryScreen shows "Back to Trail" as the primary action for trail games (verify and ensure consistency)
- [x] **POST-02**: VictoryScreen for free play games offers a "Go to Trail" option alongside existing navigation

## Future Requirements

Deferred to future milestones. Tracked but not in current roadmap.

### Content Expansion

- **CONTENT-01**: Grand Staff integration (cross-clef learning)
- **CONTENT-02**: Adaptive difficulty system
- **CONTENT-03**: Music Discovery section
- **CONTENT-04**: Two-hand basics trail nodes

### Engagement

- **ENG-01**: Celebration sound effects with volume control
- **ENG-02**: Weekly bonus events (Double XP, Speed Week)
- **ENG-03**: Spaced repetition "Rusty Skills" system
- **ENG-04**: Story campaign wrapper

### Platform

- **PLAT-01**: Production deployment to Google Play / Apple App Store

## Out of Scope

Explicitly excluded. Documented to prevent scope creep.

| Feature                               | Reason                                               |
| ------------------------------------- | ---------------------------------------------------- |
| New game modes                        | Not navigation-related; defer to future milestone    |
| Trail content changes                 | No new nodes or exercises — purely navigation/layout |
| Teacher dashboard changes             | Teachers have separate routing; out of scope         |
| First-time student onboarding changes | Existing onboarding tour stays as-is                 |

## Traceability

Which phases cover which requirements. Updated during roadmap creation.

| Requirement | Phase | Status  |
| ----------- | ----- | ------- |
| NAV-01      | —     | Pending |
| NAV-02      | —     | Pending |
| NAV-03      | —     | Pending |
| NAV-04      | —     | Pending |
| DASH-01     | —     | Pending |
| DASH-02     | —     | Pending |
| DASH-03     | —     | Pending |
| DASH-04     | —     | Pending |
| POST-01     | 19    | Done    |
| POST-02     | 19    | Done    |

**Coverage:**

- v3.1 requirements: 10 total
- Mapped to phases: 2
- Unmapped: 8 ⚠️

---

_Requirements defined: 2026-04-05_
_Last updated: 2026-04-05 — POST-01, POST-02 completed (Phase 19)_
