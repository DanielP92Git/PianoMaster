...
must_haves:
  truths:
    ...
    - "Fanfare sound plays when user clicks Stage 1 Continue button (user gesture satisfies autoplay policy)"
...
<task type="auto">
  ...
  **Stage 2: UNLOCK**
  ...
  - Play fanfare sound: call playFanfare() inside the Stage 1 Continue button's onClick handler, right before transitioning to Stage 2 (this satisfies browser autoplay policy since it's a direct user gesture)
